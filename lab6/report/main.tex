...
\begin{itemize}
    ...
    \item \textbf{LedController}: Drives the green/red LED pair according to alert state.
    ...
    \item \textbf{MonitorApp}: Orchestrates acquisition, filtering, threshold reference update, reporting, and optional alert indication.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Buzzer--DHT11 interference}: In Wokwi simulation, \texttt{tone()} may disturb DHT11 timing. The buzzer is therefore silenced briefly before each DHT11 read and restored afterwards.
...
T-12 & Memory check & Flash 45.7\%, RAM 57.6\% (both $<$ 100\%) & PASS \\
...
All 14 test scenarios passed. The most informative tests were the spike-rejection and weighted-smoothing scenarios, because they directly validate the analogue-conditioning objective of Part 2.
...
\caption{Wokwi simulation: normal operation with filtered values shown on LCD}
...
\caption{Wokwi simulation: filtered over-threshold condition using the same hardware indicators}
\label{fig:wokwi-alert}
\end{figure}

\subsection{Representative Serial Output}

The following output is representative of a Wokwi simulation run in which the NTC channel is disturbed by noise and then rises above the reference threshold:

\begin{verbatim}
Lab6 - Part2 analog conditioning monitor
Same Wokwi diagram as lab5: NTC(A0), DHT11(D8), LCD, LEDs, buzzer, POT(A1)
...
t=500ms
  ...
  TH set= 27.5C on>= 28.5C off<= 26.5C pot= 512 SYS=0
t=1000ms
  ...
  TH set= 27.8C on>= 28.8C off<= 26.8C pot= 530 SYS=0
t=1500ms
...
\begin{enumerate}
    ...
    \item \textbf{Observable intermediate states}: STDIO output exposes raw, saturated, median, and weighted values, making the conditioning effect directly visible.
...
\begin{enumerate}
    \item \textbf{Technological analysis \& context}: Covered in Domain Analysis.
    \item \textbf{Architectural design \& HW--SW explanations}: Covered through architecture figures, pipeline table, and electrical description.
    \item \textbf{Project structure \& modular implementation}: Covered in Project Directory Structure and Software Module Descriptions.
    \item \textbf{Hardware component description}: Each component is documented with role and reuse rationale.
    \item \textbf{Software module description}: Each module is listed and explained with its responsibility.
    \item \textbf{Functional implementation}: Requirements and test tables demonstrate implementation completeness.
    \item \textbf{STDIO usage}: The \texttt{printf} pipeline through \texttt{StdioBridge} is detailed and representative output is included.
    \item \textbf{Results presentation}: Diagram exports and simulation photos are included as visual evidence.
    \item \textbf{Report documentation}: Report includes analysis, design, implementation, testing, conclusions, bibliography, AI usage note, and source-code annex.
    \item \textbf{Optional extra functionality}: Bonus feature preserved and documented: potentiometer-controlled dynamic threshold reference (A1).
\end{enumerate}
...
\bigskip
\noindent PlantUML diagram source files reused from the previous lab structure are stored in:\\
