...
\begin{titlepage}
  \begin{center}
    ...
    \textsc{\large Laboratory Work \#8 - Part 2}\\[0.5cm]
...
\fancyhf{}
\rhead{\thepage}
\lhead{FAF-233 Sava Luchian; Laboratory Work No.\ 8, Part 2}
...
\hspace{0.8cm} Design and implement on an \textbf{Arduino Uno (ATmega328P)} a bare-metal actuator-control application for \textbf{Part 2} of the assignment, using the same integrated binary and analog actuator demonstrator adapted for the second report submission:
...
\bottomrule
\end{tabular}
\caption{Hardware components used in Lab 8}
\label{tab:hardware}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Wokwi}: simulation of Arduino Uno, LCD, and actuator demonstrators.
    \item \textbf{PlantUML}: source format used to define diagrams rendered to PNG files.
\end{itemize}
...
\bottomrule
\end{tabular}
\caption{Technical requirements for Lab 8 -- Part 1}
...
\bottomrule
\end{tabular}
\caption{Technical requirements for Lab 8 -- Part 2}
\label{tab:requirements2}
\end{table}

\subsection{Architectural Design}
\subsubsection{System Structural Architecture}
\inlinefigure[0.50\textheight]{layers.png}{Layered software architecture for Lab 8}{fig:layers}
\subsubsection{HW/SW Interface and Component Interaction}
\inlinefigure[0.50\textheight]{interaction.png}{Component interaction diagram for Lab 8}{fig:interaction}
...
\bottomrule
\end{tabular}
\caption{Actuator control pipeline used in Lab 8}
\label{tab:pipeline}
\end{table}
...
\inlinefigure[0.45\textheight]{electrical_schema.png}{Electrical schematic / Wokwi wiring used in Lab 8}{fig:electrical-schema}
\inlinefigure[0.45\textheight]{hardware.jpg}{Physical hardware setup used for Lab 8, Part 2}{fig:hardware-photo}
...
\begin{itemize}
    ...
    \item \textbf{Stepper28BYJ48Actuator}: generates a non-blocking full-step sequence for a 28BYJ-48 through a ULN2003-style driver path.
...
The firmware implementation referenced by this report is stored in the repository under \texttt{lab7/src/}. The Part 2 report in \texttt{lab8/report/} documents the same verified project from the perspective of the second submission part.
...
\bottomrule
\end{tabular}
\caption{PlatformIO build resource usage (Lab 8)}
\label{tab:build}
\end{table}
...
T-15 & End-to-end variant behaviour & Binary and analog actuator control coexist in one project & PASS \\
\bottomrule
\end{tabular}
\caption{Test execution results -- Lab 8}
...
\hspace{0.8cm} Laboratory Work \#8 achieved its objectives. The final project combines binary and analog actuator control in a single Arduino Uno application and therefore supports the second report submission with the same verified hardware/software baseline.
...
\begin{enumerate}
    ...
    \item \textbf{MiKTeX-ready documentation}: the report structure mirrors the previous laboratory while keeping a separate Part 2 submission folder.
\end{enumerate}

\textbf{Possible improvements:}
\begin{itemize}
    \item Use the physical 28BYJ-48 5V + ULN2003 hardware stage as the primary validation platform for all future tests.
...
\begin{enumerate}
    ...
    \item Lab 7 and Lab 8 theory/task notes on actuator control and command-signal conditioning. Technical University of Moldova, 2026.
\end{enumerate}
...
\bigskip
\noindent This report documents the same implementation stored under \texttt{lab7/src/}, while the Part 2 report sources are stored under \texttt{lab8/report/}.
